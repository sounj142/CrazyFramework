...
namespace CrazyFramework.BlazoriseClient.Services
{
	public abstract class ServiceBase
	{
		...
		{
			return Request<TResult>(() => _httpClient.PostAsJsonAsync(url, data));
		}

		public Task Post<T>(string url, T data)
		{
			return Request(() => _httpClient.PostAsJsonAsync(url, data), null);
		}

		public Task Put<T>(string url, T data)
		{
			return Request(() => _httpClient.PutAsJsonAsync(url, data), null);
		}
